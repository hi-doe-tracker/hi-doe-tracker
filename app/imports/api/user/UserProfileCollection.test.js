...
if (Meteor.isServer) {
  describe('UserProfileCollection', function testSuite() {
      ...
      fc.assert(
        fc.property(
          ...
          (firstName, lastName) => {
            ...
            const docID = UserProfiles.define({ email, firstName, lastName, password, position, assignedOffice });
    ...
    it('Cannot define duplicates', function test2() {
      ...
      const password = faker.internet.password();
      const docID1 = UserProfiles.define({ email, firstName, lastName, password, position, assignedOffice });
      const docID2 = UserProfiles.define({ email, firstName, lastName, password, position, assignedOffice });
